module Core
  class PatientState < ActiveRecord::Base
    ...
    belongs_to :patient_program, :class_name => 'Core::PatientProgram', :conditions => {:voided => 0}
    belongs_to :program_workflow_state, :foreign_key => :state, :class_name => 'Core::ProgramWorkflowState'
#, :conditions => {:retired => 0}
    ...
    def to_s
      workflow_state = Core::ProgramWorkflowState.find_state state
