...
# Is this used?
  ...
  def void
    @user = Core::User.find(params[:user_id]) rescue nil
    @order = Core::Order.find(params[:order_id])
    @order.void
  ...
  def create
    ...
    if params[:filter] and !params[:filter][:provider].blank?
      ...
      user_person_id = Core::User.find_by_user_id(params[:user_id]).person_id
    end
    ...
    @diagnosis = Core::Observation.find(params[:diagnosis]) rescue nil
    ...
    @suggestions.each do |suggestion|
      unless (suggestion.blank? || suggestion == '0' || suggestion == 'New Prescription')
        ...
        if prescriptions[:type_of_prescription] == "variable"
          Core::DrugOrder.write_order(@encounter,
                                @patient,
                                @diagnosis,
                                @drug,
                                start_date,
  ...
  def generics
    ...
    @drug_concepts = Core::ConceptName.find(:all,
  ...
  def durations
    ...
    amounts = []
    orders = Core::DrugOrder.find(:all,
  ...
  def suggested
    ...
    @orders = Core::DrugOrder.find_common_orders(@diagnosis.value_coded)
  ...
  def generic_advanced_prescription
    ...
    @user = params[:user_id]
    @patient = Core::Patient.find(params[:patient_id] || session[:patient_id]) rescue []
    ...
    @generics.each { | generic |
      drugs = Core::Drug.find(:all,	:conditions => ["concept_id = ?", generic[1]])
      drug_formulations = {}
      drugs.each { | drug |
  ...
  def create_advanced_prescription
    @user = params[:user_id]
    @patient    = Core::Patient.find(params[:encounter][:patient_id]  || session[:patient_id]) rescue nil
    @program = Core::Program.find_by_concept_id(Core::ConceptName.find_by_name("chronic care program").concept_id)
    session_date = session[:datetime].to_date rescue Date.today
    type = Core::EncounterType.find_by_name("TREATMENT") rescue nil
    ...
    if params[:prescription].blank?
      ...
      if prescription[:type_of_prescription] == "variable"
        Core::DrugOrder.write_order(encounter, @patient, nil, drug, start_date, auto_expire_date, [prescription[:morning_dose],
                                                                                             prescription[:afternoon_dose], prescription[:evening_dose], prescription[:night_dose]],
                              prescription[:type_of_prescription], prn)
      else
        Core::DrugOrder.write_order(encounter, @patient, nil, drug, start_date, auto_expire_date, prescription[:dose_strength],
    ...
    patient.encounters.current.all(:include => [:observations]).map{|encounter|
      encounter.observations.all(
          :conditions => ["obs.concept_id = ? OR obs.concept_id = ?",
                          Core::ConceptName.find_by_name("DIAGNOSIS").concept_id,
                          Core::ConceptName.find_by_name("DIAGNOSIS, NON-CODED").concept_id])
    }.flatten.compact
  end
  ...
  def new_prescription
    ...
    @drugs = Core::Drug.find(:all,:limit => 20)
    @drug_sets = {}
    @set_names = {}
    @set_descriptions = {}
    ...
    GeneralSet.find_all_by_status("active").each do |set|
      ...
      dsets.each do |d_set|
        ...
        drug = Core::Drug.find(d_set.drug_inventory_id)
  ...
  def search_for_drugs
    drugs = {}
    Core::Drug.find(:all, :conditions => ["name LIKE (?)",
  ...
  def prescribes
    ...
    params[:drug_formulations].each do |prescription|
      ...
      drug = Core::Drug.find(prescription[:drug_id])
      dose = prescription[:dose] rescue drug.dose_strength
      Core::DrugOrder.write_order(encounter, @patient, nil, drug, session_date, auto_expire_date, dose,
